...
Grid::setup_cells(void) {
    ...
    Vector3D p0 = find_min_bounds();
    Vector3D p1 = find_max_bounds();
    ...
    for (i = 0; i < nelems; i++) {  // there are only two elements in our files: vertices and faces
        ...
        if (equal_strings ((char*)"vertex", elem_name)) {
            ...
            for (j = 0; j < num_elems; j++) {
                ...
                mesh_ptr->vertices.push_back(Vector3D(vertex_ptr->x, vertex_ptr->y, vertex_ptr->z));
                delete vertex_ptr;
            }
        }
...
Grid::compute_mesh_normals(void) {
    ...
    for (int index = 0; index < mesh_ptr->num_vertices; index++) {   // for each vertex
        Vector3D normal;    // is zero at this point

        for (int j = 0; j < mesh_ptr->vertex_faces[index].size(); j++)
            normal = normal + objects[mesh_ptr->vertex_faces[index][j]]->get_normal();
...
Grid::tessellate_flat_sphere(const int horizontal_steps, const int vertical_steps) {
    ...
    for (int j = 0; j <= horizontal_steps - 1; j++) {
        ...
        Vector3D v0(	0, 1, 0);																		// top (north pole)

        Vector3D v1(	sin(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps), 			// bottom left
                    cos(pi * k / vertical_steps),
                    cos(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps)	);

        Vector3D v2(	sin(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * k / vertical_steps), 		// bottom  right
                    cos(pi * k / vertical_steps),
    ...
    for (int j = 0; j <= horizontal_steps - 1; j++) {
        ...
        Vector3D v0(	sin(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps), 			// top left
                    cos(pi * k / vertical_steps),
                    cos(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps)	);

        Vector3D v1(	0, -1, 0);																		// bottom (south pole)

        Vector3D v2(	sin(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * k / vertical_steps), 		// top right
                    cos(pi * k / vertical_steps),
                    cos(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * k / vertical_steps)	);
    ...
    for (int k = 1; k <= vertical_steps - 2; k++) {
        for (int j = 0; j <= horizontal_steps - 1; j++) {
            ...
            Vector3D v0(	sin(2.0 * pi * j / horizontal_steps) * sin(pi * (k + 1) / vertical_steps), 				// bottom left, use k + 1, j
                        cos(pi * (k + 1) / vertical_steps),
                        cos(2.0 * pi * j / horizontal_steps) * sin(pi * (k + 1) / vertical_steps)	);

            Vector3D v1(	sin(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * (k + 1) / vertical_steps), 		// bottom  right, use k + 1, j + 1
                        cos(pi * (k + 1) / vertical_steps),
                        cos(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * (k + 1) / vertical_steps)	);

            Vector3D v2(	sin(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps), 					// top left, 	use k, j
                        cos(pi * k / vertical_steps),
                        cos(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps)	);
            ...
            v0 = Vector3D(	sin(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * k / vertical_steps), 			// top right, use k, j + 1
                          cos(pi * k / vertical_steps),
                          cos(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * k / vertical_steps) );

            v1 = Vector3D (	sin(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps), 				// top left, 	use k, j
                           cos(pi * k / vertical_steps),
                           cos(2.0 * pi * j / horizontal_steps) * sin(pi * k / vertical_steps)	);

            v2 = Vector3D (	sin(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * (k + 1) / vertical_steps), 	// bottom  right, use k + 1, j + 1
                           cos(pi * (k + 1) / vertical_steps),
                           cos(2.0 * pi * (j + 1) / horizontal_steps) * sin(pi * (k + 1) / vertical_steps)	);
...
Grid::hit(const Ray& ray, double& t, ShadeRec& sr) const {
    ...
    if (bbox.inside(ray.o)) {  			// does the ray start inside the grid?
        ...
        Vector3D p = ray.o + t0 * ray.d;  // initial hit point with grid's bounding box
