...
Vector3D MeshTriangle::get_normal(void) const {
    return (normal);
}

BBox MeshTriangle::get_bounding_box(void) {
    double delta = KEPSILON;

    Vector3D v1(mesh_ptr->vertices[index0]);
    Vector3D v2(mesh_ptr->vertices[index1]);
    Vector3D v3(mesh_ptr->vertices[index2]);
...
bool MeshTriangle::shadow_hit(const Ray& ray, double& tmin) const {
    Vector3D v0(mesh_ptr->vertices[index0]);
    Vector3D v1(mesh_ptr->vertices[index1]);
    Vector3D v2(mesh_ptr->vertices[index2]);
